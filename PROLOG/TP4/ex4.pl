...
find_flights(Origin,Dest,Acc,Flights) :-    flight(Origin,Dest,_,_,_,_), append(Acc,[Dest],Flights).
...
find_flights_breadth([Dest | _], Dest, _V, Acc, Flights) :- append(_,[[Dest|T]|_],Acc), append([],[Dest|T],P), invert(P,Flights).
